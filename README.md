<p align="center">
  <img src="assets/sheepdog-logo.jpg" alt="Sheepdog AI" width="120">
</p>

<h1 align="center">Sheepdog AI</h1>

<p align="center">
  小羊成卷发布页
</p>

<p align="center">
  <a href="README.en.md">English</a>
  ·
  <a href="https://sheepdogai.net/">官网</a>
  ·
  <a href="https://sheepdogai.net/downloads">下载中心</a>
  ·
  <a href="INTRODUCTION.md">详细介绍</a>
  ·
  <a href="https://github.com/sheepdog-ai/xiaoyangchengjuan/issues">反馈问题</a>
</p>

---

## 小羊成卷

<p align="center">
  <img src="assets/lambfolio-app-icon-1.0.0.png" alt="小羊成卷 App Icon" width="132">
</p>

小羊成卷是一个给长篇小说创作者使用的 macOS 写作工具。英文名暂定为 **Lambfolio**。

它不是普通文本编辑器，而是围绕“小说生产流程”设计：把立意、设定、人物、世界观、卷纲、章节与正文修订拆成一个个可以确认、回退、微调的节点，帮助创作者把散落的灵感逐步整理成可推进的作品。

关键词：小说创作、AI 写作、长篇小说、网文写作、世界观设定、人物设定、故事大纲、章节规划、正文修订、macOS 写作工具。

### 它能帮你做什么

- **项目设定**：整理题材、核心卖点、读者预期和创作边界。
- **世界观搭建**：沉淀世界规则、地理、势力、文化与基础设定。
- **人物设计**：梳理主角、配角、关系、动机与成长线。
- **故事大纲**：生成卷纲、章节骨架和关键转折。
- **正文推进**：结合已有设定与上下文继续生成、修订和校对。
- **人工确认**：每个关键产物都可以先看、再确认；不满意可以微调或重来。

### 怎么使用

1. 从官网下载最新的 `.dmg` 安装包。
2. 打开 DMG，将 `小羊成卷.app` 拖入 `Applications`。
3. 打开应用，在设置中配置你自己的模型服务和 API Key。
4. 新建项目，按引导卡逐步填写信息。
5. 每一步生成后先确认，再推进到下一阶段。

当前版本仍在早期迭代中，更适合愿意一起打磨流程的创作者使用。

### 使用平台

- macOS 14.0 或更高版本。
- 当前发布包为 Apple Silicon / arm64 版本。
- Intel Mac 暂未单独提供构建包。

### 下载

[前往官网下载中心](https://sheepdogai.net/downloads)

备用下载入口：[GitHub Releases](https://github.com/sheepdog-ai/xiaoyangchengjuan/releases/latest)

当前发布文件：

```text
xiaoyangchengjuan-1.0.0-12.dmg
```

SHA256：

```text
dffcda19e4c870311ab55bc9c5eda2c9b22a7763f1559533bce30b00f4f72ea9
```

### 反馈与建议

如果你在使用中遇到问题，或者有很具体的改进建议，欢迎在这里留言：

[提交 Issue / 反馈建议](https://github.com/sheepdog-ai/xiaoyangchengjuan/issues)

也可以先看这篇更完整的介绍：[我做了一个给长篇小说作者用的 AI 写作工作台：小羊成卷](INTRODUCTION.md)

建议反馈时尽量带上：

- 哪一步卡住了，页面显示了什么。
- 使用的是哪家模型服务、哪个模型。
- 你期待它怎么改会更顺手。
- 是否有崩溃日志、截图或复现步骤。

我会在有空的时候继续完善和更新。这个项目会慢慢长，不急，但会一直往更好用的方向走。

### 官网

官网已上线：[sheepdogai.net](https://sheepdogai.net/)

下载中心：[sheepdogai.net/downloads](https://sheepdogai.net/downloads)


### 许可

本软件仅限个人非商业使用。禁止商用、转载、镜像、二次分发、转售或转授权。

详细条款见 [LICENSE.txt](LICENSE.txt)。
